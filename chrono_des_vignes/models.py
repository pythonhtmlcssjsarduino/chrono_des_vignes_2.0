...
from sqlalchemy_utils import ColorType as ColorType_sql_utils# type: ignore[import-untyped]
from colour import Color# type: ignore[import-untyped]
from flask_login import UserMixin
from datetime import datetime, timedelta
from chrono_des_vignes.lib import calc_points_dist
from typing import Iterator, Iterable, Literal, NamedTuple, TYPE_CHECKING, Any, Type
from markdown import markdown
from sqlalchemy import not_, Table, Integer, ForeignKey

if TYPE_CHECKING:
    from flask_sqlalchemy.query import Query# type: ignore[import-untyped]
    class Model:
...
def get_column_max_length(table:Type[Model], column_name:str) -> int:
    for column in table.__table__.columns:# type: ignore[attr-defined]
        if column.name == column_name:
            return column.type.length#type: ignore[no-any-return]
    return int(float('inf'))

class ColorType(ColorType_sql_utils):# type: ignore[misc]
    STORE_FORMAT='hex_l'
...
editions_parcours: Table = db.Table(
    'editions_parcours',
    db.Column('edition_id', Integer, ForeignKey('edition.id')),
    db.Column('parcours_id', Integer, ForeignKey('parcours.id')),
)

passagekey_stand:Table = db.Table(
    'passagekey_stand',
    db.Column('passage_key_id', Integer, ForeignKey('passage_key.id')),
    db.Column('stand_id', Integer, ForeignKey('stand.id')),
)

class User(UserMixin, Model):# type: ignore[misc]
    ...
    inscriptions = db.relationship('Inscription', back_populates='inscrit', lazy='dynamic')
    admin = db.Column(db.Boolean, nullable=False, default=False)
    creations=db.relationship('Event', back_populates='createur', lazy='dynamic')
...
class Event(Model):
    ...
    parcours=db.relationship('Parcours', back_populates='event', lazy='dynamic')
    editions=db.relationship('Edition', backref='event', lazy='dynamic')
    inscrits=db.relationship('Inscription', back_populates='event', lazy='dynamic')
    createur_id=db.Column(Integer, db.ForeignKey('user.id'), nullable=False)
    createur = db.relationship('User', back_populates='creations')
    ...
    def get_unique_inscrits(self)->list[Inscription]:
        uniques =[]
        ids =set()
...
class Parcours(Model):
    ...
    traces = db.relationship('Trace', back_populates='parcours', foreign_keys='Trace.parcours_id', lazy ='dynamic')
    ...
    name= db.Column(db.String(40), nullable=False)
    event_id=db.Column(Integer, db.ForeignKey('event.id'), nullable=False)
    event = db.relationship('Event', back_populates='parcours')
    ...
    def __len__(self)-> int:
        return len(iter(self))#type: ignore[arg-type]
    ...
    def iter_chrono_list(self)->Iterator[Stand]:
        for id in eval(self.chronos_list):
            yield Stand.query.get(id)#type: ignore[misc]
...
class Stand(Model):
    ...
    start_trace=db.relationship('Trace', back_populates='start', foreign_keys='Trace.start_id', lazy='dynamic')
    # traces qui finissent a ce stand
    end_trace=db.relationship('Trace', back_populates='end', foreign_keys='Trace.end_id', lazy='dynamic')
...
class Trace(Model):
    ...
    id = db.Column(Integer, primary_key=True)
    name= db.Column(db.String(40), nullable=False)
    parcours_id = db.Column(Integer, db.ForeignKey('parcours.id'))
    parcours = db.relationship('Parcours', back_populates='traces')
    start_id=db.Column(Integer, db.ForeignKey('stand.id'), nullable=False)
    start = db.relationship('Stand', back_populates='start_trace', foreign_keys=[start_id])
    end_id=db.Column(Integer, db.ForeignKey('stand.id'), nullable=False)
    end = db.relationship('Stand', back_populates='end_trace', foreign_keys=[end_id])
    trace = db.Column(db.Text, nullable=False, default='[]')
    turn_nb = db.Column(Integer, nullable=False)
    ...
    def get_next_trace(self)->Trace:
        if self.end == self.parcours.start_stand:
            return self.end.start_trace.filter_by(turn_nb=self.turn_nb+1).first()#type:ignore[no-any-return]
        else:
            return self.end.start_trace.filter_by(turn_nb=self.turn_nb).first()#type:ignore[no-any-return]

    def get_last_trace(self)->Trace:
        if self.start == self.parcours.start_stand:
            return self.start.end_trace.filter_by(turn_nb=self.turn_nb-1).first()#type:ignore[no-any-return]
        else:
            return self.start.end_trace.filter_by(turn_nb=self.turn_nb).first()#type:ignore[no-any-return]
...
class Edition(Model):
    ...
    inscriptions=db.relationship('Inscription', back_populates='edition', lazy='dynamic')
    ...
    passage_keys=db.relationship('PassageKey', back_populates='edition', foreign_keys='PassageKey.edition_id', lazy='dynamic')
...
class Inscription(Model):
    ...
    inscrit = db.relationship('User', back_populates='inscriptions')
    event_id=db.Column(Integer, db.ForeignKey('event.id'), nullable=False)
    event = db.relationship('Event', back_populates='inscrits')
    edition_id=db.Column(Integer, db.ForeignKey('edition.id'), nullable=False)
    edition = db.relationship('Edition', back_populates='inscriptions')
    parcours_id=db.Column(Integer, db.ForeignKey('parcours.id'), nullable=False)
    parcours:Parcours
    dossard=db.Column(Integer)
    ...
    def get_last_passage(self)->Passage:
        return self.passages.order_by(Passage.time_stamp.desc()).first()#type:ignore[no-any-return]
    
    def get_first_passage(self)->Passage:
        return self.passages.order_by(Passage.time_stamp.asc()).first()#type:ignore[no-any-return]
    ...
    def start_time(self)->datetime:
        return self.get_first_passage().time_stamp#type:ignore[no-any-return]

    @property
    def last_time(self)->datetime:
        return self.get_last_passage().time_stamp#type:ignore[no-any-return]
    ...
    @property
    def rank(self)->int|Literal['abandon', 'disqual', 'absent']|None:
        if self.end != 'finish':
            return {'abandon':'abandon', 'disqual':'disqualifié', 'absent':'absent'}.get(self.end, None)#type:ignore[return-value]
        ...
        return inscriptions.count()+1#type:ignore[no-any-return]
...
class PassageKey(Model):
    ...
    event_id=db.Column(Integer, db.ForeignKey('event.id'), nullable=False)
    edition_id=db.Column(Integer, db.ForeignKey('edition.id'), nullable=False)
    edition = db.relationship('Edition', back_populates='passage_keys')
...
class Passage(Model):
    ...
    key_id=db.Column(Integer, db.ForeignKey('passage_key.id'), nullable=True) # pas de key implique demmaré par l'admin
    key:PassageKey
    inscription_id = db.Column(Integer, db.ForeignKey('inscription.id'), nullable=False)
    ...
    def get_stand(self)->Stand:
        ...
        return stand#type:ignore[no-any-return]