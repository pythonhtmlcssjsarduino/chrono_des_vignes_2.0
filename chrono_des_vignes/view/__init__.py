...
from chrono_des_vignes.models import Event, Inscription, Edition, Parcours
...
def view_edition_page(event, edition):
    ...
    event = Event.query.filter_by(name=event).first_or_404(_('view.error.eventdontexist:event').format(event=event))
    edition:Edition = event.editions.filter_by(name=edition).first_or_404(_('view.error.editiondontexist:edition').format(edition=edition))
